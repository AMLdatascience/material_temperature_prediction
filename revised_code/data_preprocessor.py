...
def input_data_organizer(raw_data):
    ...
    xg_data = raw_data[['date', 'air_temp', 'wind_speed',
                        'rain', 'humidity', 'rail_temp', 'rail_direction']]
    ...
    xg_data['TSI'] = gr_off['date'].apply(
        lambda x: feature_preprocessor.TSI(x))
    # Azimuth, Altitude
    xg_data[['azimuth', 'altitude']] = gr_off['date'].apply(
        lambda x: pd.Series(feature_preprocessor.el_az_changer(x)))
